...
def save_nested_to_file():
    with open(full_dishes_file_path, 'w+') as f:
        header = "Day,Dish,Price\n"
        f.write(header)

        for day in nested_dish_dict:                    # Outer dictionary
            for dish_name in nested_dish_dict[day]:     # Inner dictionary
                # For every element in the inner dictionary, do a specialized formatting and write the data to the csv file
                data = f'{day},{dish_name},{nested_dish_dict[day][dish_name]}\n'
                f.write(data)
            
def edit_dishes_dict(food_and_price_dict, food_selection, selected_day):
    clear()

    selected_food = ""
    count = 0

    for food in food_and_price_dict:
        # To determine the selected_food name given an index, loop is terminated if match is found
        if count == food_selection:
            selected_food = food
            break

        count += 1

    print(f"1. Change [{selected_food}] name to something else")
    print(f"2. Change [{selected_food}] price to something else")
    ...
    if option == 1:
        ...
        new_dish_name = input("Please enter the new dish name: ").strip()

        if new_dish_name != "":                                                                 # If the new_dish_name isn't empty
            if all(char.isalpha() or char.isspace() for char in new_dish_name):
                food_and_price_dict[new_dish_name] = food_and_price_dict.pop(selected_food)     # To change a given key name
            
                print(f"\nUpdated [{selected_food}] to [{new_dish_name}]!")

                save_nested_to_file()                                                           # After changing the said key name, saves the nested dictionary to file
                pause()

            else:
                print("\nNumeric inputs are not accepted!")
                mini_pause()
        ...
        try:
            new_price = float(input("Please enter the new price: ").strip())

            old_price = food_and_price_dict[selected_food]                                      # Getting the old price
            food_and_price_dict[selected_food] = new_price                                      # Changing of key values given a selected key

            print("\nUpdated price from $%.2f to $%.2f !" %(old_price, new_price))
...
def edit_dishes(selected_day):
    food_and_price_dict = nested_dish_dict[selected_day]
    max_dish = len(food_and_price_dict)
    ...
    while True:
        try:
            ...
            count = 0
            for food in food_and_price_dict:
                # Display's all the food for the selected day including the price
                food_format = food
                food_format = food_format.ljust(24)

                price = food_and_price_dict[food]
                print("%d. %s\t$%.2f" % (count +1, food_format, price))
                count += 1

            food_selection = int(input("\nSelect the food to edit or enter [0] to go back to the previous menu: "))
        
            if food_selection == 0:                                 # If users chooses to go back to the previous menu
                break

            elif food_selection < 0 or food_selection > max_dish:   # To prevent out of bound error
                print(f"\nValid inputs are only from 1 to {max_dish}!")
                mini_pause()

            else:
                edit_dishes_dict(food_and_price_dict, food_selection -1, selected_day)
...
def edit_daily_dishes(): 
    while True: 
        try:
            ...
            print_header("Edit Daily Dishes")
            
            for day in range(0,7):                          # Range of monday to sunday -> 7 days
                print(f"{day +1}. {calendar.day_name[day]}")

            choice = int(input("\nSelect a specific day to edit or enter [0] to go back to the main menu: "))

            if choice == 0:                                 # If user chooses to go back to the main menu
                break

            elif choice > 0 and choice < 8:                 # If the selected range is between 1 to 7
                selected_day = calendar.day_name[choice -1] # Choice -1 because 0 represent monday and 6 represents sunday
                edit_dishes(selected_day)

            else:
                print("\nValid inputs are from 1-7")
                mini_pause()

        except ValueError:                                  # If user enters any other input besides numbers
            print("\nOnly accepts numeric values!")
