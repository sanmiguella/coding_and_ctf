...
class Security: # Superclass.
    ...
    def generate_encrypted_data(self, data):
        ...
        client_pub_key = self.get_key_from_file(self.client_public_key)
        b64_encoded_rsa_encrypted_session_key = b64encode(self.rsa_encrypt(b64decode(b64_encoded_session_key), client_pub_key)).decode(self.default_encoding)
    ...
    def decrypt_received_data(self, encrypted_data):
        ...
        if verified:
            server_priv_key = self.get_key_from_file(self.server_private_key)
            b64_encoded_session_key = b64encode(self.rsa_decrypt(b64decode(b64_encoded_rsa_encrypted_session_key), server_priv_key))
    ...
    def get_rsa_signature(self, data):
        server_priv_key = self.get_key_from_file(self.server_private_key)
        rsa_signature_b64_encoded_iv_and_ciphertext = self.rsa_sign(data, server_priv_key)
       
        return rsa_signature_b64_encoded_iv_and_ciphertext

    def verify_rsa_signature(self, signature, data):
        client_pub_key = self.get_key_from_file(self.client_public_key)
        verification_results = self.rsa_verify(b64decode(data), b64decode(signature), client_pub_key)
